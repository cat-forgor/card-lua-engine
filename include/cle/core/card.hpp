...
public:
  ...
  Card(const Card &other)
      : name_{other.name_}, type_{other.type_}, mana_cost_{other.mana_cost_},
        colors_{other.colors_}, oracle_text_{other.oracle_text_},
        flavor_text_{other.flavor_text_}, subtypes_{other.subtypes_},
        keywords_{other.keywords_}, creature_stats_{other.creature_stats_},
  ...
  [[nodiscard]] auto mana_cost() const -> const ManaCost & {
    return mana_cost_;
  }
  [[nodiscard]] auto colors() const -> Color { return colors_; }
  [[nodiscard]] auto oracle_text() const -> const std::string & {
    return oracle_text_;
  }
  [[nodiscard]] auto flavor_text() const -> const std::string & {
    return flavor_text_;
  }
  [[nodiscard]] auto subtypes() const -> const std::vector<std::string> & {
    return subtypes_;
  }
  [[nodiscard]] auto keywords() const -> const std::vector<std::string> & {
    return keywords_;
  }
